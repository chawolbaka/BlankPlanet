...
public class OxygenHooks {
	...
	/**
	 * Test whether a bounding box (normally a block but it could be an entity) is inside an oxygen bubble or oxygen sealed space, on an otherwise oxygen-free world. (Do not use this on the Overworld or other oxygen-rich world,, it will return false
	 * negatives!!)
	 * 
	 * NOTE: In a complex build where this block is surrounded by air-permeable blocks on all sides (for example torches, ladders, signs, wires, chests etc etc) then it may have to look quite far to find whether it is in oxygen or not - it will check
	 * up to 5 blocks in each direction. This can impose a performance load in the unlikely event there are permeable blocks in all directions. It is therefore advisable not to call this every tick: 1 tick in 5 should be plenty.
	 * 
	 * @param world
	 *            The World
	 * @param bb
	 *            AxisAligned BB representing the block (e.g. a torch), or maybe the side of a block
	...
	/**
	 * Special version of the oxygen AABB check for living entities. This is based on checking the oxygen contact of a small box centred at the entity's eye height. The small box has sides equal to half the width of the entity. This is a good
	 * approximation to head size and position for most types of mobs.
	 * 
	 * @param entity
	...
	/**
	 * Simplified (better performance) version of the block oxygen check for use with torch blocks and other oxygen-requiring blocks which can access oxygen on any side.
	 * 
	 * NOTE: this does not run an inOxygenBubble() check, you will need to do that also.
	 * 
	 * @param world
	 * @param block
	 *            The block type of this torch being checked - currently unused
	 * @param x
	 *            x coordinate of the torch
	 * @param y
	 *            y coordinate of the torch
	 * @param z
	 *            z coordinate of the torch
	 ...
	 *            World
	 * @param avgX
	 *            avg X, avgY, avgZ are the average co-ordinates of the location
	 * @param avgY
	 *            (for example, the central point of a block being tested
	 * @param avgZ
	 *            or the average position of the centre of a living entity)
