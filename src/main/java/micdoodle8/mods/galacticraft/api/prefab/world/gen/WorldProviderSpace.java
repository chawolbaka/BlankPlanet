...
public abstract class WorldProviderSpace extends WorldProvider implements IGalacticraftWorldProvider {
	 ...
	 * 
	 * Returns true on clients (to allow rendering of sky etc, maybe even clouds). Returns false on servers (to disable Nether Portal mob spawning and sleeping in beds).
	 */
	@Override
	...
	/**
	 * This must normally return false, so that if the dimension is set for 'static' loading it will not keep chunks around the dimension spawn position permanently loaded. It is also needed to be false so that the 'Force Overworld Respawn' setting
	 * in core.conf will work correctly - see also WorldProviderS[ace.getRespawnDimension().
	 * 
	 * But: returning 'false' will cause beds to explode in this dimension. If you want beds NOT to explode, you can override this, like in WorldProviderMoon.
	 */
	@Override
	 ...
	 * 
	 * This controls whether the player will respawn in the space dimension or the Overworld in accordance with the 'Force Overworld Respawn' setting on core.conf.
	 */
	@Override
	 ...
	 * 
	 * Obeying the 'Force Overworld Respawn' setting from core.conf is an important protection for players are endlessly dying in a space dimension: for example respawning in an airless environment with no oxygen tanks and no oxygen machinery.
	 */
	...
	/**
	 * If false (the default) then Nether Portals will have no function on this world. Nether Portals can still be constructed, if the player can make fire, they just won't do anything.
	 * 
